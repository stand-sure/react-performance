...
function dogReducer(state, action) {
  if (action.type === "TYPED_IN_DOG_INPUT") {
...
function AppProvider({children}) {
  ...
  return (
    <AppStateContext.Provider value={gridState}>
      <AppDispatchContext.Provider value={gridDispatch}>
        {children}
...
function withStateSlice(Component, slice) {
  ...
  function Wrapper(props, ref) {
    ...
    return (
      <MemoizedComponent ref={ref} state={slice(state, props)} {...props} />
    );
  }
  ...
  return (
    <button
      ...
      style={{
        ...
        backgroundColor: `rgba(128, 0, 128, ${cell / 100})`,
      }}
    >
      {Math.floor(cell)}
    </button>
